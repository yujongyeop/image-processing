...
def OpenImg(num):
    ...
    if img is None:
        ...
        FindDeadPixel(img, num, img2)


def FindDeadPixel(img, num, img2):
    totalDeadPixel = 0
    center_status = True
    (b, g, r) = cv2.split(img)
    b[:, :] = 0
    ...
    mask2 = cv2.threshold(img2, 0, 255, cv2.THRESH_BINARY)[1]
...
def ShowImg(mask, status, num):
    ...
    if status == 'NG':
        ...
        cv2.putText(mask, status, (2350, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, green)
